...
Plataforma de inteligência de dados e análise estatística esportiva de alta precisão. Inspirada nas Moiras da mitologia grega, as tecelãs do destino.

Multi-esporte (futebol, vôlei, basquete, baseball), multi-inquilino (SaaS), com arquitetura reativa de 5 camadas (PostgreSQL → Kafka → Redis → TimescaleDB → ClickHouse), suporte a AI embeddings (pgvector), Knowledge Graph, ML Feature Store e auditoria enterprise.
...
| Auth | JWT + X-Tenant-ID + RBAC (system_role) |
...
### MOI-001 a MOI-010 — Core
Schema DDL completo com 51 tabelas, 19 ENUMs, 3 MVs, 2 views, 12 RLS policies, 8 partitioned tables. Multi-sport (football, volleyball, basketball, baseball) com SCD Type 2, JSONB contracts, audit columns.

### MOI-011 — Universal Event Engine
`sport_events` unificado com payload JSONB, particionado por RANGE. Arquitetura híbrida: write normalizado, read via MVs.

### MOI-012 — Live State Engine
`match_state_snapshots` com posse, momentum, pressure_index, fadiga, live xG. Particionado por RANGE(captured_at).

### MOI-013 — AI Embeddings Layer
`entity_embeddings` com VECTOR(384) para pgvector (cosine distance, IVFFlat). RAG esportivo.

### MOI-014 — Multi-Tenant SaaS (CRITICAL)
`organizations`, `tenants`, `tenant_users`, `tenant_permissions`, `entity_tenants` (polymorphic mapping). `tenant_id UUID` injetado em 7 tabelas core + 12 RLS policies.

### MOI-015 — Tracking & Spatial Data
`tracking_frames`, `player_coordinates`, `ball_coordinates` (25+ FPS, PARTITION BY RANGE). Suporte a Computer Vision.

### MOI-016 — Knowledge Graph (HIGH)
`graph_nodes` (vértices polimórficos), `graph_edges` (7 predicados: played_with, coached_by, rival_of, injured_in, transferred_to, agent_of, tactical_cluster). Pesos e propriedades JSONB.

### MOI-ADM — Enterprise Dashboard & Governance
`system_role` ENUM (super_admin → viewer), `audit_logs` (tamper-proof trail com 4 índices forenses + RLS). 7 módulos de dashboard (Global Ops, Tenant, Live Match, AI Analytics, Observability, Security, Media).

### MOI-LMCC — LiveMatchCommandCenter
Zustand store (`useLiveMatchStore`) com 25 FPS, WebSocket hook com backoff exponencial, Canvas renderer (`SpatialPitchRender`) com atletas por time e bola com eixo Z.

### MOI-SEC — Security Gateway
Rate limiting por tier (standard 60rpm → betting 10000rpm), Redis sliding window, middleware FastAPI para validação X-Tenant-ID + RBAC enforcement.

### MOI-CH — ClickHouse Observability
`api_network_metrics` (MergeTree, TTL 30d), `mv_api_network_performance_minutely` (SummingMergeTree), `websocket_telemetry_metrics` (ReplacingMergeTree, TTL 14d), Kafka Engine ingress + MV chain.

---

## API Routes (16 rotas, 0 erros no build)

| Rota | Método | Descrição |
|------|--------|-----------|
| `/api/matches` | GET | Lista/detalhe partidas |
| `/api/players` | GET | Lista/detalhe atletas |
| `/api/competitions` | GET/POST | Competições / classificação |
| `/api/scanner` | GET | Partidas escaneadas |
| `/api/standings` | GET | Classificação |
| `/api/v1/matches/live` | GET | Partidas ao vivo (Redis) |
| `/api/v1/ai/scouting-report` | POST | Scout via pgvector + grafo |
| `/api/live/match/{id}` | GET | Snapshot + WebSocket fallback |

---

## Páginas

`/` Dashboard · `/matches` Partidas · `/matches/[id]` Detalhe + LiveMatchTracker · `/players` Atletas · `/players/[id]` Perfil + Radar SVG · `/compare` Comparador · `/scanner` Scanner · `/competitions` Classificação
...
├── components/
│   ├── LiveMatchTracker.tsx # Simulação ao vivo (693 linhas)
│   └── tactical/
│       └── SpatialPitchRender.tsx # Canvas 25 FPS
├── data/seed.ts             # Mock data (5 comps, 23 times, 18 players, 14 matches + all entities)
├── database/
│   ├── schema.sql           # 51 tables, 19 ENUMs, 12 RLS policies
│   ├── clickhouse_observability.sql  # ClickHouse DDL
│   └── migration_athletes.sql
├── hooks/
│   └── useMatchWebSocket.ts # WebSocket resiliente
├── middleware/
│   ├── tenant_boundary.py   # FastAPI: X-Tenant-ID validation
│   └── rbac_enforcer.py     # FastAPI: RBAC enforcement
├── services/
│   ├── predictionEngine.ts  # Poisson prediction
│   └── scannerService.ts    # Live scanner
├── store/
│   └── useLiveMatchStore.ts # Zustand (25 FPS)
├── types/
│   ├── sports.ts            # Domain types (279 linhas)
│   └── database.ts          # DB mirror types (87 exports, 1216 linhas)
├── utils/
│   ├── mathEngine.ts        # Statistical functions
│   └── financeEngine.ts     # EV + Kelly Criterion
└── map.md                   # Documentação completa da arquitetura
```
...
> O conteúdo abaixo é reproduzido de [`map.md`](map.md), a fonte única da verdade arquitetural do MoirAI Sports Engine.
...
```json
{
  ...
  "overall": 89,
  "spider_6": {
    "pace": 91, "shooting": 83, "passing": 85,
    "dribbling": 95, "defending": 30, "physical": 52
  },
  "spider_12": {
    "pace": 91, "shooting": 83, "passing": 85,
    "dribbling": 95, "defending": 30, "physical": 52,
    "stamina": 73, "vision": 88, "agility": 92,
    "acceleration": 94, "composure": 82, "leadership": 65
  }
}
```
...
| **Módulos puros**      | `mathEngine` e `financeEngine` sem efeitos colaterais |
...
├──────────────────────────────────────────────────────────────┤
│  moirai-ml      → pgvector + Knowledge Graph                │
│                  embeddings, similaridade, scout AI          │
├──────────────────────────────────────────────────────────────┤
│  moirai-analytics → ClickHouse + TimescaleDB                │
│                  MVs, relatórios, dashboards históricos      │
└──────────────────────────────────────────────────────────────┘
```
...
- **LiveMatchCommandCenter**: Zustand store, WebSocket hook com backoff exponencial, Canvas renderer
- **MoirAISecurityGateway**: Rate limiting (60/1200/10000 rpm), tenant_boundary.py + rbac_enforcer.py
- **ClickHouse Observability**: MergeTree, SummingMergeTree, ReplacingMergeTree, Kafka Engine DDL
- **Dependências**: zustand adicionado
- **Build**: 16 routes, 0 errors

### 2026-05-27 (v9) — v0.3.5-Admin · MOI-ADM
- **RBAC Enterprise**: system_role ENUM (7 papéis) + coluna em tenant_users
- **Audit Trail**: audit_logs table com 4 índices forenses + RLS
- **Tipos TS**: SystemRole, AuditLog, systemRole em TenantUser
- **Seed**: 5 audit_logs
- **Schema**: 51 tabelas, 19 ENUMs, 12 RLS policies

### 2026-05-27 (v8) — v0.3.5-Release
- ML Feature Store, API Gateway v1, arquitetura 5 camadas, infra docs (Kafka, Redis, TimescaleDB, ClickHouse), Event Lifecycle Contract

### 2026-05-27 (v7)
- Knowledge Graph Layer (graph_nodes + graph_edges), Event Versioning (sport_events_v3), RLS refinado (10 policies), entity_tenants mapping

### 2026-05-27 (v6)
- Universal Event Engine (sport_events), Live State Engine (match_state_snapshots), AI Embeddings (entity_embeddings), Multi-Tenant SaaS, Tracking & Spatial Data

### 2026-05-27 (v5)
- MOI-001 a MOI-010: circular DDL fix, player_attributes, audit columns, SCD Type 2, JSONB contracts, media_assets, odds, GenAI readiness, Big Data strategy, 4-layer architecture

### 2026-05-27 (v4)
- Staff, injuries, transfers, lineups, rankings, materialized views, indexes, partitioning docs

### 2026-05-26 (v2)
- 7 pages, 5 API routes, 12 Brazilian teams, 10 players, radar chart, LiveMatchTracker, scanner UI, dark theme

### 2026-05-26 (v1)
- Initial scaffold: Next.js 14 + TypeScript + Tailwind v3 + dark theme
